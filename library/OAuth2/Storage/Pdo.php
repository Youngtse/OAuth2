    ...
    OpenIDAuthorizationCodeInterface
{
    ...
    public function setClientDetails($client_secret = null, $redirect_uri = null, $client_name = null , $client_logo_url = null, $client_homepage = null, $grant_types = null, $scope = null, $user_id = null)
    {
...
//        }
        $stmt = $this->db->prepare(sprintf('INSERT INTO %s (client_secret, redirect_uri, client_name , client_logo_url, client_homepage, grant_types, scope, user_id) VALUES (:client_secret, :redirect_uri, :client_name, :client_logo_url, :client_homepage, :grant_types, :scope, :user_id)', $this->config['client_table']));
        return $stmt->execute(compact('client_secret', 'redirect_uri', 'client_name', 'client_logo_url', 'client_homepage', 'grant_types', 'scope', 'user_id'));
    }
    ...
    //是否存在access_token
    public function isTokenExist($user_id, $client_id)
    {
        $stmt = $this->db->prepare(sprintf('SELECT  COUNT(*) from %s where client_id = :client_id AND user_id = :user_id', $this->config['access_token_table']));
    ...
    public function setAccessToken($access_token, $client_id, $user_id, $expires, $scope = null)
    {
        ...
        // if it exists, update it.
        if ($this->getAccessToken($access_token) || $this->isTokenExist($user_id, $client_id)) {
            $stmt = $this->db->prepare(sprintf('UPDATE %s SET access_token=:access_token, expires=:expires, scope=:scope where client_id=:client_id and user_id=:user_id', $this->config['access_token_table']));
        } else {
